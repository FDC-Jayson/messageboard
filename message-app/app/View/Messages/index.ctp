...
<script>
    ...
    app.config(function($routeProvider) {
        $routeProvider
        .when("/", {
            templateUrl: "/messages/htmlMainTemplate",
            controller: "listCtrl"
        })
        .when("/send", {
            templateUrl: "/messages/htmlSendTemplate",
            controller: "sendCtrl"
        })
        .when("/details/:toUserId", {
            templateUrl: "/messages/htmlDetailsTemplate",
    ...
    /** List Controller */
    app.controller("listCtrl", function($scope, $http, $location, $timeout) {
        $scope.messages = [];
        $scope.totalMsg = 0;
        $scope.page = 1;
        $scope.hasMore = false;
        $scope.getMessages = function() {
            $http({
                method: "GET",
                url: "/api/messages/" + "<?php echo $userData['id'] ?>/" + $scope.page
        ...
        $scope.deleteConversation = function(toUserId) {
            ...
            $http({
                ...
                url: "/api/messages-delete-conversation/" + toUserId,
            }).then(function mySuccess(response) {
                ...
                if (response.data) {
                    ...
                    if (index !== -1) {
                        ...
                        $(`#conversation-${toUserId}`).fadeOut('fast', function() {
    ...
    /** Send Controller */
    app.controller("sendCtrl", function($scope, $http, $location) {
    ...
    /** Details Controller */
    app.controller("detailsCtrl", function($scope, $http, $routeParams, $timeout, $location) {
        ...
        $scope.deleteMessage = function(event, deleteType, data) {
            ...
            userIds.forEach(userId => {
                $http({
                    method: "POST",
                    headers: { 'Content-Type': 'application/json' },
                    url: "api/message-delete/" + data.Message.id + "/" + userId,
                }).then(function mySuccess(response) {
                    // Assuming response.data.success is true when the deletion is successful
                    if (response.data) {
        ...
        $scope.searchMessages = function(toUserId) {
            ...
            $http({
                method: "POST",
                headers: { 'Content-Type': 'application/json' },
                url: "api/messages-search-count/" + toUserId + "/" + searchQuery,
            }).then(function mySuccess(response) {
    ...
    /** Custom Directives */
    app.directive('select2', function($http) {
        return {
            restrict: 'A',
